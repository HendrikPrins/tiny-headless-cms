class ImageAssetTool {
    ...
    _renderInner() {
        ...
        pickBtn.addEventListener('click', () => {
            if (window.ImageAssetPicker) {
                window.ImageAssetPicker.open((asset) => {
                    this.data.assetId = asset.id;
                    this.data.url = asset.url;
                    this.data.filename = asset.filename;
                    // Preserve existing alt/caption
                    this._renderInner();
                });
            }
        });
        ...
        if (hasImage) {
            ...
            capInput.addEventListener('input', () => { this.data.caption = capInput.value; });
